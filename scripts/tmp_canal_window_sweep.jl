...
const OUTDIR = joinpath(@__DIR__, "..", "outputs", "window_sweep")
...
function _canal_orientation_map_param(Pmap::AbstractMatrix, Δx::Real, Δy::Real;
        ...
        tensor_smooth_radius_pix::Int=4, score_quantile::Real=0.95, anis_min::Real=3.0)
    P = Float64.(Pmap)
    valid = isfinite.(P)
...
function analyze_case(Pmap::AbstractMatrix, b1::AbstractMatrix, b2::AbstractMatrix, bperp::AbstractMatrix;
        local_radius_pix::Int, ridge_smooth_radius_pix::Int, Δ::Real)
...
function main()
    ...
    mkpath(OUTDIR)
    out_csv = joinpath(OUTDIR, "canal_window_sweep.csv")
    open(out_csv, "w") do io
        println(io, "map_tag,local_window,ridge_window,mean_abs_delta_deg,median_abs_delta_deg,frac_parallel_15deg,npix,mean_crossings,median_crossings,sigma_parallel_deg,median_abs_parallel_deg,frac_parallel_family_15deg,theta_dom_deg")
        for (map_tag, Pmap) in maps
            for rl in local_radii
                for rr in ridge_radii
                    stats = analyze_case(Pmap, b1, b2, bperp; local_radius_pix=rl, ridge_smooth_radius_pix=rr, Δ=paths.lbox_pc / 256.0)
                    @printf(io, "%s,%d,%d,%.6f,%.6f,%.6f,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
                        map_tag, 2 * rl + 1, 2 * rr + 1,
